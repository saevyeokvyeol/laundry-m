...
public class SuccessView {
	...
	public static void printUserBook(List<Book> books) {
		...
		for (Book book : books) {
			System.out.println(book.getBookId() + " | " + book.getLaundry().getLaundryName() + " | " + book.getBookCount() + "벌 | " + won.format(book.getBookTotalFee()) + "원 | " + book.getBookMethod().getBookMethodName() + " | " + getDate(book.getBookInsertDate()) + " | " + book.getBookState().getBookStateName());
			for (BookLine bookLine : book.getBookLine()) {
				System.out.println("  ㄴ" + clothes[bookLine.getClothesId().intValue()] + " | " + fabric[bookLine.getFabricId().intValue()] + " | " + won.format(bookLine.getBookLineFee()) + "원");
			}
			System.out.println();
		}
	}
